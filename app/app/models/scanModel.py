...
class ScanModel(db.Model):
    ...
    @classmethod
    def update_status_finish_time(cls, scan_id):
        cls.query.filter_by(id=scan_id).update({'status': 1, 'finish_time': datetime.now()})
        db.session.commit()

    @classmethod
    def find_by_status(cls, status, length=10):
        if status == 0:
            return cls.query.filter_by(status=False).order_by(ScanModel.create_time.desc()).all()
        else:
            print('find by status')
            return cls.query.filter_by(status=True).order_by(ScanModel.create_time.desc()).limit(length).all()
